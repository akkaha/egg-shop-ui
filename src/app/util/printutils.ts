import { OrderBill, ShopOrder } from '../model/egg.model';
...
export function printShopOrder(order: ShopOrder, bill: OrderBill) {
  const content = `
<pre style="padding:5px;margin:0px;">
  单号: ${order.id}\t时间: ${order.createdAt.substr(0, 10)}\t姓名: ${order.user}
